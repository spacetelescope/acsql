...
a means for users to discover and interact with these data via a database-driven web application.  This is accomplished via several
\texttt{acsql} components: (1) A $\sim$40 TB network file system, which stores all on-orbit ACS data files on disk, (2) a
...
based on obseravational parameters, (3) A \texttt{Python}/\texttt{Flask}-based web application, which allows users to view
``Quicklook'' JPEG images of any publicly-available ACS data along with its metadata, and (4) a \texttt{Python} code library, which
...
At this time, the web application is not yet hosted on a dedicated web server (neither externally nor
...
3. Possible extensions to the current implementation
